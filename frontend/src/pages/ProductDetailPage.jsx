...
const ProductDetailPage = () => {
    ...
    const handleAddToCart = async (e) => {
        ...
        if (currentInventory === 0) return showToast("این کالا در حال حاضر ناموجود است.", "danger");
        if (quantity > currentInventory) return showToast(`حداکثر موجودی قابل سفارش ${currentInventory} عدد است.`, "warning");
    ...
    const handleCompareClick = (e) => {
        e.preventDefault();
        if (product) addToCompare(product.uuid);
    };
    ...
    const schemaMarkup = product.seo?.schema_markup || product.json_ld || product.seo?.json_ld;
    ...
    return (
        <React.Fragment>
            ...
            <main className="product-detail-page" itemScope itemType="https://schema.org/Product">
                ...
                <meta itemProp="dateModified" content={product.modified_at || product.created_at} />

                <nav className="bread-crumb py-0 mb-3 mt-3" aria-label="راهنمای مسیر">
                    <div className="container-fluid">
                        <div className="content-box border-0 shadow-sm rounded-4 bg-white">
                            <ol className="breadcrumb mb-0 py-3 px-3 px-md-4">
                                <li className="breadcrumb-item"><Link to="/" className="font-13 font-md-14 text-muted text-decoration-none hover-text-danger transition"><i className="bi bi-house me-1" aria-hidden="true"></i>خانه</Link></li>
                    ...
                    <div className="container-fluid">
                        <div className="content-box bg-transparent p-0 border-0">
                            <div className="row gy-4">
                                ...
                                <section className="col-12 col-lg-4" aria-label="گالری تصاویر محصول">
                                    <div className="pro_gallery position-relative bg-white p-3 p-md-4 rounded-4 shadow-sm border border-ui h-lg-100">
                                        ...
                                        <div className="position-absolute z-3 top-0 start-0 mt-4 ms-4 d-flex flex-column gap-2">
                                            {product.is_special_offer && <span className="badge bg-danger shadow-sm px-3 py-2 rounded-pill font-13 fw-bold animate-pulse d-flex align-items-center gap-1"><i className="bi bi-lightning-charge-fill text-warning fs-6" aria-hidden="true"></i> پیشنهاد شگفت‌انگیز</span>}
                                                ...
                                                <SwiperSlide key={idx} aria-label={`تصویر ${idx + 1} از ${allImages.length}`}>
                                                    <div className="swiper-zoom-container bg-white">
                                                        <img 
                                                            ...
                                                            src={img.url} 
                                                            alt={img.image_alt || `تصویر کالا ${product.title}`} 
                                ...
                                <section className="col-12 col-lg-4" aria-label="اطلاعات اصلی محصول">
                                    <div className="product-meta bg-white p-4 p-md-5 rounded-4 shadow-sm border border-ui h-lg-100 d-flex flex-column">
                                        {product.brand && (
                                            <div className="mb-3" itemProp="brand" itemScope itemType="https://schema.org/Brand">
                                                <Link itemProp="url" to={`/shop?brands=${product.brand.slug}`} className="text-danger font-14 fw-bold text-decoration-none hover-text-dark transition d-flex align-items-center gap-2 w-fit-content px-3 py-1 bg-danger bg-opacity-10 rounded-pill" aria-label={`محصولات برند ${product.brand.title}`}>
                                                    <i className="bi bi-tag-fill" aria-hidden="true"></i> <span itemProp="name">{product.brand.title}</span>
                                                </Link>
                                            </div>
                                        )}
                                        ...
                                        {product.expert_reviewer && (
                                            <div className="alert bg-success bg-opacity-10 border border-success border-opacity-25 rounded-pill py-2 px-3 d-inline-flex align-items-center gap-2 mb-3 shadow-sm w-fit-content">
                                                <i className="bi bi-patch-check-fill text-success fs-5" aria-hidden="true"></i>
                                        ...
                                        <div className="d-flex align-items-center pb-4 border-bottom border-light flex-wrap gap-3 gap-md-4">
                                            ...
                                            <div className="d-flex align-items-center text-muted font-13" aria-label={`${product.comments?.length || 0} دیدگاه ثبت شده`}>
                                                <i className="bi bi-chat-text me-2" aria-hidden="true"></i> {product.comments?.length || 0} دیدگاه
                                            </div>
                                            <div className="d-flex align-items-center text-success font-13 fw-bold" aria-label={`فروخته شده: ${product.sold_count} عدد`}>
                                                <i className="bi bi-cart-check-fill me-2" aria-hidden="true"></i> {product.sold_count} فروش
                                            </div>
                                        </div>

                                        <div className="product-feature py-4">
                                            <h2 className="font-16 mb-3 fw-bold text-dark d-flex align-items-center gap-2"><i className="bi bi-card-checklist text-primary" aria-hidden="true"></i> ویژگی‌های برتر</h2>
                                            ...
                                            {product.key_takeaways && Array.isArray(product.key_takeaways) && product.key_takeaways.length > 0 ? (
                                                <ul className="list-unstyled m-0 p-0 d-flex flex-column gap-3 mb-4">
                                                    {product.key_takeaways.map((kt, idx) => (
                                                        <li key={idx} className="font-13 text-dark d-flex align-items-start fw-semibold"><i className="bi bi-check-circle-fill text-success me-2 fs-5" aria-hidden="true"></i>{kt}</li>
                                                    ))}
                                                </ul>
                                        ...
                                        <div className="alert bg-primary bg-opacity-10 border border-primary border-opacity-25 rounded-4 p-3 d-flex align-items-start gap-3 mt-auto mb-0" role="note">
                                ...
                                <section className="col-12 col-lg-4" aria-label="باکس خرید و قیمت">
                                    <div className="alert border border-ui p-3 p-md-4 rounded-4 shadow-sm bg-white glass-panel sticky-sidebar" itemProp="offers" itemScope itemType="https://schema.org/Offer">
                                        ...
                                        <div className="d-flex bg-light border border-ui alert align-items-center justify-content-between rounded-4 mb-4">
                                            <div className="form-check m-0 d-flex align-items-center gap-2">
                                                <input className="form-check-input mt-0 shadow-none cursor-pointer" style={{width:'18px', height:'18px'}} type="checkbox" id="insuranceCheck" checked={hasInsurance} onChange={(e) => setHasInsurance(e.target.checked)} aria-label="انتخاب بیمه کالا" />
                                        ...
                                        {product.is_variable && Object.keys(availableAttributes).map((attrName, index) => {
                                            ...
                                            return (
                                                <div key={index} className="mb-4">
                                                    <div className="font-14 fw-bold mb-3 d-block text-dark d-flex align-items-center gap-2"><i className="bi bi-sliders text-secondary" aria-hidden="true"></i> انتخاب {attrName}</div>
                                                    <div className="d-flex flex-wrap gap-2" role="radiogroup" aria-label={`انتخاب ویژگی ${attrName}`}>
                                                            ...
                                                            return (
                                                                <React.Fragment key={val}>
                                                                    ...
                                                                    <label className={`btn font-12 px-3 py-2 border rounded-pill transition hover-lift ${isSelected ? 'border-danger text-danger bg-danger bg-opacity-10 fw-bold shadow-sm' : 'border-ui text-muted bg-white'}`} htmlFor={`attr-${attrName}-${val}`}>
                                                                        {isColor && <span className="d-inline-block rounded-circle me-2 border border-ui shadow-sm" style={{width:'12px', height:'12px', verticalAlign:'middle', backgroundColor: val === 'مشکی' ? '#212529' : val === 'سفید' ? '#f8f9fa' : val === 'قرمز' ? '#dc3545' : val === 'سبز' ? '#198754' : val === 'آبی' ? '#0d6efd' : '#ccc'}} aria-hidden="true"></span>}
                                                                        {val}
                                                                    </label>
                                                                </React.Fragment>
                                                            );
                                        ...
                                        <div className="text-start my-4 bg-light p-3 rounded-4 border border-light position-relative overflow-hidden">
                                            ...
                                            <div className={`text-muted font-13 mb-2 d-flex align-items-center justify-content-between ${product.is_special_offer ? 'mt-4 pt-3' : ''}`}>
                                                <span>قیمت کالا:</span>
                                                {hasDiscount && <del className="text-danger" aria-label="قیمت اصلی">{Number(originalPrice).toLocaleString()}</del>}
                                            </div>
                                            <div className="fs-3 fw-900 text-dark d-flex align-items-center justify-content-between m-0">
                                                ...
                                                <span className="text-success d-flex align-items-center gap-2">
                                                    ...
                                                    <span aria-label={`قیمت نهایی: ${Number(currentPrice).toLocaleString()} تومان`}>{Number(currentPrice).toLocaleString()}</span> <span className="text-muted font-13 fw-normal ms-1" aria-hidden="true">تومان</span>
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <form onSubmit={handleAddToCart} className="w-100 d-flex flex-column align-items-center gap-3">
                                            {product.is_wholesale && (
                                                <div className="font-12 text-dark text-center bg-warning bg-opacity-25 py-2 px-3 rounded-pill w-100 border border-warning border-opacity-50" role="alert">
                                                    <i className="bi bi-info-circle-fill text-warning me-2" aria-hidden="true"></i>حداقل خرید عمده: <strong className="text-danger fw-bold">{product.wholesale_min_quantity} عدد</strong>
                                                </div>
                                            )}
                                            ...
                                            <div className="counter bg-white border border-ui rounded-pill p-2 d-flex align-items-center justify-content-between w-100 shadow-sm">
                                                <span className="font-13 fw-bold text-muted ms-3" id="qty-label">تعداد کالا:</span>
                                                <div className="d-flex align-items-center bg-light rounded-pill border border-light p-1">
                                                    <button type="button" className="btn btn-sm btn-white rounded-circle shadow-sm border-0 d-flex align-items-center justify-content-center" style={{width:'32px', height:'32px'}} onClick={() => setQuantity(prev => Math.max(1, prev - 1))} disabled={quantity <= 1 || currentInventory === 0} aria-label="کاهش تعداد محصول"><i className="bi bi-dash text-danger fw-bold" aria-hidden="true"></i></button>
                                                    <input id="product-qty" aria-labelledby="qty-label" type="text" className="form-control border-0 text-center bg-transparent fw-900 font-16 p-0" value={quantity} readOnly style={{width:'35px'}} aria-live="polite"/>
                                                    <button type="button" className="btn btn-sm btn-white rounded-circle shadow-sm border-0 d-flex align-items-center justify-content-center" style={{width:'32px', height:'32px'}} onClick={() => setQuantity(prev => Math.min(currentInventory, prev + 1))} disabled={quantity >= currentInventory || currentInventory === 0} aria-label="افزایش تعداد محصول"><i className="bi bi-plus text-success fw-bold" aria-hidden="true"></i></button>
                                                </div>
                                            </div>

                                            {currentInventory > 0 ? (
                                                <button type="submit" disabled={isAddingToCart} className="btn main-color-two-bg w-100 text-center text-white rounded-pill shadow hover-lift py-3 font-16 fw-bold d-flex align-items-center justify-content-center gap-2" aria-label="تایید و افزودن این کالا به سبد خرید">
                                                ...
                                                <button type="button" className="btn btn-secondary w-100 text-center text-white rounded-pill disabled py-3 fw-bold font-16" disabled aria-label="کالا ناموجود است">
                                                    ناموجود
                                                </button>
                                            )}
                                        </form>
                    ...
                    <div className="container-fluid">
                        <div className="bg-white rounded-4 shadow-sm border border-ui p-2 mb-4 d-flex justify-content-center overflow-auto custom-scrollbar">
                            <ul className="nav nav-pills flex-nowrap m-0 p-0 gap-2" role="tablist" aria-label="منوی اطلاعات تکمیلی">
                                ...
                                <li className="nav-item flex-shrink-0" role="presentation">
                                    <button id="tab-specs" aria-controls="panel-specs" className={`nav-link rounded-pill px-4 py-2 font-14 transition fw-bold d-flex align-items-center gap-2 ${activeTab === 'specs' ? 'active bg-danger text-white shadow-sm' : 'text-muted hover-bg-light border border-transparent'}`} onClick={() => setActiveTab('specs')} role="tab" aria-selected={activeTab === 'specs'}>
                                        <i className="bi bi-sliders2" aria-hidden="true"></i> مشخصات فنی
                                    </button>
                                </li>
                                <li className="nav-item flex-shrink-0" role="presentation">
                                    <button id="tab-comments" aria-controls="panel-comments" className={`nav-link rounded-pill px-4 py-2 font-14 transition fw-bold d-flex align-items-center gap-2 ${activeTab === 'comments' ? 'active bg-danger text-white shadow-sm' : 'text-muted hover-bg-light border border-transparent'}`} onClick={() => setActiveTab('comments')} role="tab" aria-selected={activeTab === 'comments'}>
                                        <i className="bi bi-chat-dots" aria-hidden="true"></i> نظرات
                                        <span className={`badge rounded-pill ms-1 ${activeTab === 'comments' ? 'bg-white text-danger' : 'bg-secondary bg-opacity-25 text-dark'}`} aria-label={`تعداد نظرات: ${product.comments?.length || 0}`}>{product.comments?.length || 0}</span>
                                    </button>
                                </li>
                                <li className="nav-item flex-shrink-0" role="presentation">
                                    <button id="tab-qa" aria-controls="panel-qa" className={`nav-link rounded-pill px-4 py-2 font-14 transition fw-bold d-flex align-items-center gap-2 ${activeTab === 'qa' ? 'active bg-danger text-white shadow-sm' : 'text-muted hover-bg-light border border-transparent'}`} onClick={() => setActiveTab('qa')} role="tab" aria-selected={activeTab === 'qa'}>
                                        <i className="bi bi-question-circle" aria-hidden="true"></i> پرسش و پاسخ
                                        <span className={`badge rounded-pill ms-1 ${activeTab === 'qa' ? 'bg-white text-danger' : 'bg-secondary bg-opacity-25 text-dark'}`} aria-label={`تعداد پرسش‌ها: ${product.questions?.length || 0}`}>{product.questions?.length || 0}</span>
                                    </button>
                                </li>
                            </ul>
                        </div>
                            ...
                            <div className="col-xl-10">
                                <div className="content-box rounded-4 p-4 p-md-5 border-ui shadow-sm bg-white min-vh-50">
                                    ...
                                    <div className="product-descs">
                                        {activeTab === 'desc' && (
                                            <section id="panel-desc" aria-labelledby="tab-desc" className="product-desc-content animate-fade-in" role="tabpanel">
                                                <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                                                    <div className="bg-danger bg-opacity-10 text-danger rounded-4 d-flex align-items-center justify-content-center" style={{ width: '50px', height: '50px' }}>
                                                        <i className="bi bi-file-earmark-text fs-4" aria-hidden="true"></i>
                                                    </div>
                                        ...
                                        {activeTab === 'specs' && (
                                            <section id="panel-specs" aria-labelledby="tab-specs" className="animate-fade-in" role="tabpanel">
                                                <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                                                    <div className="bg-danger bg-opacity-10 text-danger rounded-4 d-flex align-items-center justify-content-center" style={{ width: '50px', height: '50px' }}>
                                                        <i className="bi bi-sliders2-vertical fs-4" aria-hidden="true"></i>
                                                    </div>
                                        ...
                                        {activeTab === 'comments' && (
                                            <section id="panel-comments" aria-labelledby="tab-comments" className="product-comment-content animate-fade-in" role="tabpanel">
                                                <div className="comment-form mb-5 bg-light border border-ui rounded-4 p-4 p-md-5 shadow-sm">
                                                    <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                                                        <div className="bg-danger bg-opacity-10 text-danger rounded-4 d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: '50px', height: '50px' }}>
                                                            <i className="bi bi-chat-quote-fill fs-4" aria-hidden="true"></i>
                                                        </div>
                                                        <div>
                                                    ...
                                                    {user ? (
                                                        <form onSubmit={handleCommentSubmit} aria-label="فرم ثبت نظر">
                                                            <div className="row gy-4">
                                                                        ...
                                                                        <div className="d-flex flex-row-reverse rating-stars fs-4" role="radiogroup" aria-label="امتیاز کالا از 1 تا 5">
                                                                            {[5, 4, 3, 2, 1].map(star => (
                                                                                <React.Fragment key={star}>
                                                                                    <input type="radio" name="rating" id={`star${star}`} value={star} checked={rating === star} onChange={() => setRating(star)} aria-label={`امتیاز ${star} ستاره`} />
                                                                ...
                                                                <div className="col-12">
                                                                    ...
                                                                    <textarea id="commentBody" className="form-control border-0 shadow-sm py-3 px-4 rounded-4 font-13" rows="4" placeholder="کامل‌ترین تجربه خود را بنویسید..." value={commentBody} onChange={(e) => setCommentBody(e.target.value)} required aria-required="true"></textarea>
                                                                </div>
                                                                <div className="col-12 text-end mt-4">
                                                                    <button type="submit" className="btn btn-danger px-4 px-md-5 py-2 py-md-3 rounded-pill fw-bold font-14 shadow hover-lift w-100 w-md-auto" disabled={submittingComment} aria-label="ارسال دیدگاه">
                                                        ...
                                                        <div className="alert bg-white text-center rounded-4 p-4 p-md-5 shadow-sm border-0" role="alert">
                                                ...
                                                <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light mt-5">
                                                    <div className="bg-light text-muted rounded-4 d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: '40px', height: '40px' }}>
                                                        <i className="bi bi-people-fill fs-5" aria-hidden="true"></i>
                                                    </div>
                                                ...
                                                {product.comments && product.comments.length > 0 ? (
                                                    <div className="d-flex flex-column gap-3 gap-md-4">
                                                        {product.comments.map(comment => {
                                                            ...
                                                            return (
                                                                <article key={comment.uuid} className="box_users_comment border border-light rounded-4 p-3 p-md-4 shadow-sm bg-white hover-shadow transition" itemScope itemProp="review" itemType="https://schema.org/Review">
                                                                    <div className="row">
                                                                        <div className="col-12 col-lg-3 border-end-lg border-light mb-3 mb-lg-0 d-flex flex-row flex-lg-column align-items-center align-items-lg-start justify-content-between">
                                                                            <div className="d-flex align-items-center gap-3 mb-0 mb-lg-3">
                                                                                ...
                                                                                <div>
                                                                                    <span className="fw-bold font-14 text-dark d-block mb-1" itemProp="author" itemScope itemType="https://schema.org/Person"><span itemProp="name">{comment.user_name}</span></span>
                                                                                    <span className="font-11 text-muted px-2 py-1 bg-light rounded-pill border"><time itemProp="datePublished" dateTime={comment.created_at}>{new Date(comment.created_at).toLocaleDateString('fa-IR')}</time></span>
                                                                                </div>
                                                                            </div>
                                                                            <div className="d-flex text-warning font-13 bg-warning bg-opacity-10 px-3 py-1 rounded-pill border border-warning border-opacity-25 mt-0 mt-lg-auto" aria-label={`امتیاز ${comment.rating} از 5`} itemProp="reviewRating" itemScope itemType="https://schema.org/Rating">
                                        ...
                                        {activeTab === 'qa' && (
                                            <section id="panel-qa" aria-labelledby="tab-qa" className="animate-fade-in" role="tabpanel" itemScope itemType="https://schema.org/FAQPage">
                                                <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light">
                                                    <div className="bg-danger bg-opacity-10 text-danger rounded-4 d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: '50px', height: '50px' }}>
                                                        <i className="bi bi-question-diamond-fill fs-4" aria-hidden="true"></i>
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="box_questions bg-light border border-light rounded-4 p-4 p-md-5 mb-5 shadow-sm">
                                                    <form onSubmit={handleQuestionSubmit} aria-label="فرم ثبت پرسش">
                                                        <div className="row gy-3">
                                                            <div className="col-12">
                                                                <div className="alert bg-info bg-opacity-10 border border-info border-opacity-25 rounded-3 d-flex align-items-start gap-2 m-0 font-13 text-dark" role="alert">
                                                            ...
                                                            <div className="col-12">
                                                                ...
                                                                <textarea id="questionText" className="form-control border-0 py-3 px-4 rounded-4 shadow-sm font-13" placeholder="سوال خود را درباره ویژگی‌ها یا کارایی این کالا بنویسید..." rows="3" value={questionText} onChange={(e) => setQuestionText(e.target.value)} required aria-required="true"></textarea>
                                                            </div>
                                                            <div className="col-12 text-end mt-4">
                                                                <button className="btn btn-dark text-white px-4 py-2 py-md-3 px-md-5 rounded-pill fw-bold font-13 shadow-sm hover-lift w-100 w-md-auto" type="submit" disabled={submittingQuestion} aria-label="ارسال پرسش به پشتیبانی">
                                                ...
                                                <div className="d-flex align-items-center gap-3 mb-4 pb-3 border-bottom border-light mt-5">
                                                    <div className="bg-light text-muted rounded-4 d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: '40px', height: '40px' }}>
                                                        <i className="bi bi-ui-checks fs-5" aria-hidden="true"></i>
                                                    </div>
                                                ...
                                                {product.questions && product.questions.length > 0 ? (
                                                    <div className="d-flex flex-column gap-4">
                                                        {product.questions.map((q) => (
                                                            <article key={q.uuid} className="box_questions border border-ui rounded-4 p-3 p-md-4 bg-white shadow-sm hover-shadow transition" itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
                                                                ...
                                                                {q.answer_text && (
                                                                    <div className="row bs-qu align-items-start pt-3 border-top border-light" itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                                                                        <div className="col-2 col-lg-2 bq1 text-center border-end border-light pe-2 pe-md-3">
            ...
            <div className="modal fade" id="videoModal" tabIndex="-1" aria-labelledby="videoModalLabel" aria-hidden="true" role="dialog" aria-modal="true">
                <div className="modal-dialog modal-lg modal-dialog-centered">
                    <div className="modal-content bg-dark border-0 overflow-hidden rounded-4 shadow-lg">
                        <header className="modal-header border-0 position-absolute top-0 end-0 z-3 w-100 p-3 d-flex justify-content-end bg-gradient-dark">
                            <h2 id="videoModalLabel" className="visually-hidden">ویدیو معرفی کالا</h2>
                            <button type="button" className="btn-close btn-close-white shadow-none" data-bs-dismiss="modal" aria-label="بستن ویدیو"></button>
                        </header>
            ...
            <div className="share-modal py-0">
                <div className="modal fade" id="shareModal" tabIndex="-1" aria-labelledby="shareModalLabel" aria-hidden="true" role="dialog" aria-modal="true">
                    <div className="modal-dialog modal-sm modal-dialog-centered">
                        <div className="modal-content border-0 rounded-4 shadow-lg overflow-hidden glass-panel">
                            <header className="modal-header bg-light border-0 px-4 py-3">
                                <h2 className="modal-title font-15 fw-900 text-dark m-0 d-flex align-items-center gap-2" id="shareModalLabel"><i className="bi bi-share-fill text-danger fs-5" aria-hidden="true"></i> اشتراک گذاری</h2>
                                <button type="button" className="btn-close shadow-none" data-bs-dismiss="modal" aria-label="بستن صفحه اشتراک گذاری"></button>
                            </header>
                            <div className="modal-body text-center p-4">
                                <p className="font-13 text-muted mb-4 lh-base">ارسال این محصول شگفت‌انگیز به دوستان!</p>
                                <button type="button" onClick={copyToClipboard} className="btn my-3 d-block text-center btn-dark w-100 rounded-pill py-2 font-14 fw-bold transition shadow-sm hover-lift" aria-label="کپی کردن لینک محصول">
                                    <i className="bi bi-files me-2 fs-5 align-middle" aria-hidden="true"></i> کپی لینک
                                </button>
                                <div className="d-flex justify-content-center gap-4 social-link fs-3 mt-4">
                                    <a href={`https://t.me/share/url?url=${window.location.href}`} target="_blank" rel="noreferrer" className="text-primary transition hover-lift" aria-label="ارسال لینک محصول در تلگرام"><i className="bi bi-telegram" aria-hidden="true"></i></a>
                                    <a href={`https://wa.me/?text=${window.location.href}`} target="_blank" rel="noreferrer" className="text-success transition hover-lift" aria-label="ارسال لینک محصول در واتس‌اپ"><i className="bi bi-whatsapp" aria-hidden="true"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="modal fade" id="chartModal" tabIndex="-1" aria-labelledby="chartModalLabel" aria-hidden="true" role="dialog" aria-modal="true">
                <div className="modal-dialog modal-dialog-centered modal-lg">
                    <div className="modal-content border-0 rounded-4 shadow-lg overflow-hidden">
                        <header className="modal-header bg-light border-0 px-4 py-3 d-flex align-items-start">
                            <div>
                                <h2 className="modal-title fw-900 text-dark font-16 d-flex align-items-center gap-2" id="chartModalLabel"><i className="bi bi-graph-up-arrow text-danger fs-4" aria-hidden="true"></i> نمودار تغییرات قیمت فروش</h2>
                                <p className="text-muted mt-2 font-12 m-0 text-overflow-1">{product.title}</p>
                            </div>
                            <button type="button" className="btn-close shadow-none mt-1" data-bs-dismiss="modal" aria-label="بستن نمودار"></button>
                        </header>
                        <div className="modal-body p-3 p-md-4 bg-white">
                            <div className="w-100 d-flex justify-content-center chart-container">
                                <canvas id="myChart" ref={chartRef} aria-label="رسم گرافیکی تغییرات قیمت محصول در طول زمان"></canvas>
                            </div>
                        </div>
                    </div>
                </div>
